    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-center bg-white p-8 rounded-4xl border-2 border-slate-100 shadow-sm">
        <div>
          ...
          <p className="text-10 font-black text-slate-400 uppercase tracking-widest mt-1">Configure Catalog & Production Strategies</p>
        </div>
        ...
        {products.map((product) => (
          <Card key={product.id} className="relative group overflow-hidden border-2 border-slate-50 hover:border-blue-100 transition-all">
            <div className="flex justify-between items-start mb-6">
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${product.deductionModel === 'ON_PRODUCTION' ? 'bg-blue-50 text-blue-600 font-bold' : 'bg-amber-50 text-amber-500 font-bold'}`} style={{ backgroundColor: product.deductionModel === 'ON_PRODUCTION' ? '#eff6ff' : '#fffbeb', color: product.deductionModel === 'ON_PRODUCTION' ? '#2563eb' : '#f59e0b' }}>
            ...
            <div className="space-y-4 border-t border-slate-50 pt-6">
              <div className="flex flex-col gap-2">
                <label className="text-10 font-black text-slate-400 uppercase tracking-widest pl-1">Deduction Method</label>
              ...
              {product.deductionModel === 'ON_PRODUCTION' && (
                <div className="bg-slate-50 p-4 rounded-2xl border-2 border-slate-100">
                  <label className="block text-9 font-black text-slate-500 uppercase tracking-widest mb-2">Pieces Per Batch</label>
                  <div className="flex items-center gap-4">
                    <input 
                      ...
                      style={{ border: '1px solid #e2e8f0', borderRadius: '0.75rem', padding: '0.5rem 1rem' }}
                    />
                    <p className="text-10 font-bold text-slate-400">PCS</p>
                  </div>
                </div>
              )}
            </div>
