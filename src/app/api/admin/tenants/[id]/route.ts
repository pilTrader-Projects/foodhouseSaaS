...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        const tenant = await adminService.getTenantDetails(params.id)
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        let result
        if (body.plan) {
            result = await adminService.updateTenantPlan(params.id, body.plan)
        } else if (body.status) {
            result = await adminService.updateTenantStatus(params.id, body.status)
        } else {
